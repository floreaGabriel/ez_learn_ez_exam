...
// ============================== 12. OPERATOR GSM (telefonie mobilă) ==============================
{
  id:"gsm", nume:"Operator telefonie mobilă (GSM)", icon:"📱",
  nivel:"Calibrat 1:1 pe Subiectul III (structura Abonați–Solicitări–Cartele). Același tipar ca „Bibliotecă universitară”, temă de operator GSM.",
  rezumat:"Sufix LIKE, a patra cifră cu <code>_</code>, minimum 2 pe an, integritate referențială — exact nivelul examenului.",
  intro:"Fie tabelele <code>Abonati</code>, <code>Solicitari</code> și <code>Cartele</code>, în care sunt păstrate informații despre abonații unui operator de telefonie mobilă, solicitările de cartele GSM și cartelele GSM emise.",
  tabele:[
    {nume:"Abonati", campuri:[
      ["Identificator (PK)","nu","int"],["Nume","nu","varchar(50)"],
      ["Prenume","nu","varchar(50)"],["Localitate","nu","varchar(50)"]]},
    {nume:"Solicitari", campuri:[
      ["Identificator (PK)","nu","int"],["ID_Abonat (FK)","nu","int"],
      ["Data","nu","datetime"],["Stare","da","char(1)"]]},
    {nume:"Cartele", campuri:[
      ["Identificator (PK)","nu","int"],["ID_Solicitare (FK)","nu","int"],
      ["Numar_Telefon","nu","varchar(10)"],["Valabila_De_La","nu","datetime"],
      ["Valabila_Pana_La","nu","datetime"],["Blocata","da","datetime"]]}
  ],
  observatii:[
    "Câmpul <code>Stare</code> al unei solicitări are valoarea <code>null</code> la creare, <code>R</code> după ce a fost emisă cartela solicitată, sau <code>X</code> dacă solicitarea a fost anulată.",
    "Câmpul <code>Blocata</code> are valoarea <code>null</code> dacă cartela poate fi folosită, sau conține <b>data</b> la care a fost blocată cartela."
  ],
  relatii:"<code>Abonati 1—N Solicitari 1—N Cartele</code>. Un abonat face mai multe solicitări; o solicitare onorată (<code>Stare='R'</code>) produce o cartelă emisă. Cele două câmpuri cu <code>null</code> (<code>Stare</code>, <code>Blocata</code>) concentrează majoritatea „capcanelor”.",
  cerinte:[
    {lit:"a", enunt:"Afișează <b>prenumele</b> abonaților al căror <b>nume</b> are ultimele 4 litere „escu”.",
     sql:"SELECT Prenume FROM Abonati WHERE Nume LIKE '%escu';",
     note:"<b>Explicație:</b> aici cerem un <b>sufix</b> (terminația), deci wildcard-ul <code>%</code> stă la <b>stânga</b>: <code>'%escu'</code> = orice șir urmat de „escu”. <b>Capcană (invers față de prefix):</b> <code>'escu%'</code> ar căuta nume care <b>încep</b> cu „escu” — greșit. Pentru un prefix („Pop…”) ai pune <code>'Pop%'</code>; pentru sufix, <code>'%escu'</code>. Prinde „Popescu”, „Ionescu”, „Georgescu”."},
    {lit:"b", enunt:"Afișează <b>numerele de telefon</b> ale cartelelor care sunt <b>blocate</b>.",
     sql:"SELECT Numar_Telefon FROM Cartele WHERE Blocata IS NOT NULL;",
     note:"<b>Explicație:</b> <code>Blocata = null</code> înseamnă „poate fi folosită”, deci o cartelă <b>blocată</b> are o <b>dată</b> în câmp → <code>IS NOT NULL</code>. <b>Capcană clasică:</b> nu se scrie <code>Blocata &lt;&gt; null</code> (comparația cu <code>null</code> dă mereu <code>UNKNOWN</code>). Obligatoriu <code>IS [NOT] NULL</code>."},
    {lit:"c", enunt:"Afișează <b>localitatea</b> cu cel mai mare număr de abonați.",
     sql:"SELECT TOP (1) Localitate FROM Abonati\nGROUP BY Localitate\nORDER BY COUNT(*) DESC;",
     note:"<b>Explicație:</b> grupăm pe localitate, numărăm abonații, ordonăm descrescător, luăm prima. <b>Capcană (egalitate la maxim):</b> dacă două localități au același maxim, <code>TOP 1</code> întoarce <b>una singură</b> (nedeterminist). Pentru toate egalitățile: <code>TOP 1 WITH TIES</code> sau <code>HAVING COUNT(*) = (SELECT MAX(...))</code>."},
    {lit:"d", enunt:"Afișează <b>solicitările rezolvate</b> care au fost făcute de către abonații care locuiesc în <b>București</b>.",
     sql:"SELECT Solicitari.* FROM Solicitari\nINNER JOIN Abonati ON Abonati.Identificator = Solicitari.ID_Abonat\nWHERE Abonati.Localitate = 'Bucuresti' AND Solicitari.Stare = 'R';",
     note:"<b>Explicație:</b> <code>INNER JOIN</code> leagă fiecare solicitare de abonat; filtrăm pe localitate și pe starea <code>'R'</code> (rezolvată). <b>Atenție la NULL:</b> <code>Stare = 'R'</code> exclude automat solicitările <code>NULL</code> (în procesare) și <code>'X'</code> (anulate) — exact ce vrem."},
    {lit:"e", enunt:"Afișează <b>numele, prenumele</b> și <b>numărul de solicitări</b> efectuate de un abonat, ordonate descrescător după numărul de solicitări ale fiecăruia.",
     sql:"SELECT Nume, Prenume, COUNT(Solicitari.Identificator) AS Nr_Solicitari\nFROM Abonati\nINNER JOIN Solicitari ON Abonati.Identificator = Solicitari.ID_Abonat\nGROUP BY Solicitari.ID_Abonat, Nume, Prenume\nORDER BY COUNT(Solicitari.Identificator) DESC;",
     note:"<b>De ce <code>ID_Abonat</code> în <code>GROUP BY</code>:</b> ca să distingem doi abonați <b>omonimi</b>. <code>Nume, Prenume</code> apar în <code>GROUP BY</code> doar ca să poată fi afișate (orice coloană neagregată din <code>SELECT</code> trebuie să fie în <code>GROUP BY</code>). <b>Observație:</b> cu <code>INNER JOIN</code>, abonații fără nicio solicitare nu apar; pentru „0 solicitări” treci la <code>LEFT JOIN</code>."},
    {lit:"f", enunt:"Afișează <b>numărul de solicitări rezolvate</b>, emise în perioada 01.01.2021–31.01.2021, pentru care <b>a patra cifră</b> a numărului de telefon (al cartelei asociate) este <b>nenulă</b>.",
     sql:"SELECT COUNT(*) FROM Solicitari\nINNER JOIN Cartele ON Solicitari.Identificator = Cartele.ID_Solicitare\nWHERE Solicitari.Stare = 'R'\n  AND (Solicitari.Data >= '2021-01-01' AND Solicitari.Data < '2021-02-01')\n  AND Cartele.Numar_Telefon LIKE '___[1-9]%';",
     note:"<b>Pe bucăți:</b> <code>Stare = 'R'</code> (rezolvate); <code>Data &gt;= '2021-01-01' AND Data &lt; '2021-02-01'</code> — am evitat intenționat <code>BETWEEN ... '2021-01-31'</code>, care ar rata orele din 31 ian. de după 00:00:00. <b>A patra cifră nenulă:</b> <code>LIKE '___[1-9]%'</code> — <b>trei</b> caractere <code>_</code> (fiecare <code>_</code> = exact un caracter oarecare) sar peste primele 3 cifre, apoi <code>[1-9]</code> cere cifra 4 ∈ {1..9} (deci nu 0), iar <code>%</code> lasă restul liber."},
    {lit:"g", enunt:"Afișează <b>numele și prenumele</b> abonaților care au făcut <b>minimum două solicitări în același an</b> (un abonat apare cel mult o singură dată în răspuns).",
     sql:"SELECT DISTINCT Nume, Prenume FROM Abonati\nINNER JOIN Solicitari ON Abonati.Identificator = Solicitari.ID_Abonat\nGROUP BY Solicitari.ID_Abonat, YEAR(Data), Nume, Prenume\nHAVING COUNT(*) >= 2;",
     note:"<b>Explicație:</b> gruparea pe <code>ID_Abonat, YEAR(Data)</code> creează câte o grupă per (abonat, an); <code>HAVING COUNT(*) &gt;= 2</code> păstrează abonații cu ≥2 solicitări într-un an. <code>DISTINCT</code> e necesar fiindcă un abonat care îndeplinește condiția în <b>mai mulți ani</b> ar apărea altfel de mai multe ori. <b>Capcană:</b> dacă am grupa doar pe <code>ID_Abonat</code> (fără <code>YEAR</code>) am număra solicitările din toți anii la un loc — greșit („în același an”)."},
    {lit:"h", enunt:"Afișează <b>numele, prenumele și numerele de telefon</b> ale abonaților care au cel puțin o cartelă <b>care nu este blocată</b>, care <b>expiră înainte</b> de 31 decembrie 2021.",
     sql:"SELECT Nume, Prenume, Numar_Telefon\nFROM Abonati\n     INNER JOIN Solicitari ON Abonati.Identificator = Solicitari.ID_Abonat\n     INNER JOIN Cartele    ON Solicitari.Identificator = Cartele.ID_Solicitare\nWHERE Cartele.Blocata IS NULL\n  AND Cartele.Valabila_Pana_La < '2021-12-31';",
     note:"<b>Explicație:</b> dublu <code>INNER JOIN</code> pe lanțul <code>Abonati → Solicitari → Cartele</code>; <code>Blocata IS NULL</code> = nu e blocată, <code>Valabila_Pana_La &lt; '2021-12-31'</code> = expiră înainte. <b>Înlănțuirea JOIN-urilor:</b> în SQL Server scrii direct mai multe <code>INNER JOIN</code> unul după altul; fiecare <code>ON</code> leagă perechea de tabele imediat anterioară. Fără <code>Blocata IS NULL</code> ai include și cartele blocate."},
    {lit:"i", enunt:"Afișează <b>numărul de telefon</b> pentru cartelele <b>„orfane”</b> (cartelele pentru care a fost ștearsă solicitarea corespunzătoare, cea care făcea legătura cu abonatul care deține cartela).",
     sql:"SELECT Numar_Telefon FROM Cartele\nWHERE ID_Solicitare NOT IN (SELECT Identificator FROM Solicitari);",
     note:"<b>Raționament (proiectare):</b> <code>Cartele.ID_Solicitare</code> e FK obligatoriu (<code>null = nu</code>); integritatea referențială <b>nu permite</b> cartele orfane în acest design (de aceea, pe datele de test, rezultatul e vid). Interogarea rezolvă cerința <i>dacă</i> BD ar permite orfani. <b>Capcană <code>NOT IN</code> + <code>NULL</code>:</b> dacă subinterogarea poate conține <code>NULL</code>, <code>NOT IN</code> întoarce vid → preferă <code>NOT EXISTS</code> sau <code>LEFT JOIN ... IS NULL</code>."}
  ],
