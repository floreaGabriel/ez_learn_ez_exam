...
// ============================== 6. HOTEL ==============================
{
  ...
  cerinte:[
    {lit:"a", enunt:"Afișează <b>prenumele</b> oaspeților al căror <b>nume începe</b> cu „Pop”.", sql:"SELECT Prenume FROM Oaspeti WHERE Nume LIKE 'Pop*';", ansi:"SELECT Prenume FROM Oaspeti WHERE Nume LIKE 'Pop%';", note:"<code>*</code> (Access) / <code>%</code> (ANSI) = orice șir după prefix."},
    ...
    {lit:"c", enunt:"Afișează <b>orașul</b> cu cei mai mulți oaspeți.", sql:"SELECT TOP 1 Oras FROM Oaspeti\nGROUP BY Oras\nORDER BY Count(*) DESC;", ansi:"... ORDER BY COUNT(*) DESC LIMIT 1;", note:"grupare + ordonare descrescătoare, prima înregistrare."},
    ...
    {lit:"h", enunt:"Afișează <b>numele, prenumele și codul facturii</b> oaspeților cu cel puțin o factură <b>neachitată</b> <b>scadentă înainte</b> de 31 decembrie 2021.", sql:"SELECT Nume, Prenume, Cod_Factura\nFROM (Oaspeti\n     INNER JOIN Rezervari ON Oaspeti.Id_Oaspete = Rezervari.Id_Oaspete)\n     INNER JOIN Facturi   ON Rezervari.Id_Rezervare = Facturi.Id_Rezervare\nWHERE Facturi.Achitata IS NULL\n  AND Facturi.Scadenta < #2021-12-31#;", note:"dublu <code>INNER JOIN</code> + <code>IS NULL</code> + comparație de date. Paranteze Access obligatorii la 2+ join-uri."},
    {lit:"i", enunt:"Afișează <b>codurile facturilor „orfane”</b> (facturi pentru care a fost ștearsă rezervarea).", sql:"SELECT Cod_Factura FROM Facturi F\nWHERE NOT EXISTS (SELECT 1 FROM Rezervari R WHERE R.Id_Rezervare = F.Id_Rezervare);", ansi:"-- LEFT JOIN ... IS NULL:\nSELECT F.Cod_Factura FROM Facturi F\nLEFT JOIN Rezervari R ON F.Id_Rezervare = R.Id_Rezervare\nWHERE R.Id_Rezervare IS NULL;", note:"FK obligatoriu → fără orfani în acest design. <code>NOT EXISTS</code> e robust la <code>NULL</code> (spre deosebire de <code>NOT IN</code>)."},
    ...
    {lit:"k", enunt:"<b>(FULL OUTER JOIN)</b> Afișează corespondența completă <b>rezervare ↔ factură</b>, inclusiv rezervările fără factură și (ipotetic) facturile fără rezervare.", sql:"SELECT Rezervari.Id_Rezervare, Facturi.Cod_Factura\nFROM Rezervari\nFULL OUTER JOIN Facturi ON Rezervari.Id_Rezervare = Facturi.Id_Rezervare;", ansi:"-- Access nu are FULL OUTER JOIN; se simulează cu UNION de LEFT + RIGHT:\nSELECT R.Id_Rezervare, F.Cod_Factura FROM Rezervari R LEFT JOIN Facturi F ON R.Id_Rezervare=F.Id_Rezervare\nUNION\nSELECT R.Id_Rezervare, F.Cod_Factura FROM Rezervari R RIGHT JOIN Facturi F ON R.Id_Rezervare=F.Id_Rezervare;", note:"<b>FULL OUTER JOIN</b> = toate rândurile din ambele tabele, cu <code>NULL</code> unde nu există corespondent. SQLite/SQL Server îl suportă; Access îl simulează prin <code>UNION</code> de <code>LEFT</code> + <code>RIGHT</code>."},
...
// ============================== 7. BANCĂ ==============================
{
  ...
  cerinte:[
    {lit:"a", enunt:"Afișează <b>prenumele</b> clienților al căror nume se termină în „escu”.", sql:"SELECT Prenume FROM Clienti WHERE Nume LIKE '*escu';", ansi:"SELECT Prenume FROM Clienti WHERE Nume LIKE '%escu';", note:"sufix: <code>*</code>/<code>%</code> la stânga."},
...
// ============================== 8. ASIGURĂRI AUTO ==============================
{
  ...
  cerinte:[
    {lit:"a", enunt:"Afișează <b>prenumele</b> asiguraților al căror <b>nume conține</b> „an”.", sql:"SELECT Prenume FROM Asigurati WHERE Nume LIKE '*an*';", ansi:"SELECT Prenume FROM Asigurati WHERE Nume LIKE '%an%';", note:"<code>*..*</code> = „conține”."},
...
// ============================== 9. CURIERAT ==============================
{
  ...
  cerinte:[
    {lit:"a", enunt:"Afișează <b>prenumele</b> expeditorilor al căror <b>nume începe</b> cu o literă din intervalul A–M.", sql:"SELECT Prenume FROM Expeditori WHERE Nume LIKE '[A-M]*';", ansi:"-- SQLite: WHERE upper(substr(Nume,1,1)) BETWEEN 'A' AND 'M'", note:"clasă de caractere <code>[A-M]</code> pe prima poziție."},
    ...
    {lit:"i", enunt:"Afișează <b>codurile AWB „orfane”</b> (livrări pentru care a fost șters coletul).", sql:"SELECT Cod_AWB FROM Livrari L\nWHERE L.Id_Colet NOT IN (SELECT Id_Colet FROM Colete);", ansi:"-- robust (NOT EXISTS):\nSELECT Cod_AWB FROM Livrari L\nWHERE NOT EXISTS (SELECT 1 FROM Colete C WHERE C.Id_Colet = L.Id_Colet);", note:"<code>NOT IN</code> e fragil la <code>NULL</code>; preferă <code>NOT EXISTS</code>."},
...
// ============================== 10. UNIVERSITATE ==============================
{
  ...
  cerinte:[
    {lit:"a", enunt:"Afișează <b>prenumele</b> studenților al căror nume se termină în „escu” <b>sau</b> „eanu”.", sql:"SELECT Prenume FROM Studenti\nWHERE Nume LIKE '*escu' OR Nume LIKE '*eanu';", ansi:"... WHERE Nume LIKE '%escu' OR Nume LIKE '%eanu';", note:"două sufixe unite prin <code>OR</code> (atenție la paranteze cu alte filtre)."},
